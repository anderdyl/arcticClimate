...
iceBmus = iceDWTs['bmus_final'][31:]
iceDWT = iceBmus

# fixedInds = np.where((iceBmus == 42))
# iceDWT[fixedInds] = 5
...
for qq in range(len(np.unique(iceDWT))):
    ...
    ax4.plot(np.array(slpDatetimes)[getBMUS[0]],qq*np.ones((len(temp),)),'.',color=dwtcolors[qq])
...
# import pickle
#
# dwtPickle = 'processedWaveWaterLevels49DWTs36IWTs2022.pickle'
# outputDWTs = {}
