...
sSims = np.arange(0,500,25)
eSims = np.arange(25,501,25)
...
for beginSim in sSims:
    print('working on {} to {}'.format(beginSim,beginSim+25))
    hs = []
    tp = []
    dm = []
    ss = []
    t2m = []
    numOfSims = 100
    getSims = np.arange(beginSim,beginSim+25)
    for hh in range(len(getSims)):
       numSim = getSims[hh]

       file = ('/volumes/macDrive/arcticSims/tuk/futureSimulation{}.pickle'.format(numSim))
    ...
    tideTuk = loadmat('/users/dylananderson/documents/projects/arcticClimate/tide_emulation_Tuk.mat')
    syntideTuk = tideTuk['synTide']
    syntimeTuk = tideTuk['synTime']
    environTimeTuk = tideTuk['synTime']#[matlab_to_datetime(t * 24 * 60 * 60) for t in tideNome['synTime']]
    environIndTuk = np.where((environTimeTuk>=simTime[0]) & (environTimeTuk<=simTime[-1]))
    tideTuk = syntideTuk[environIndTuk]
    ...
    savemat('/volumes/macDrive/arcticSims/tukSimulations{}to{}.mat'.format(beginSim+1,beginSim+25),mdict)
