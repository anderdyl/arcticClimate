...
dwtPercent = historicalDWTs['nPercent'][0:10]
dwtPercent = (dwtPercent / dwtPercent[0]) * 0.5

icePercent = historicalICEs['nPercent'][0:2]
...
while keep_iter:
    ...
    kma = KMeans(n_clusters=numClusters, n_init=1000).fit(dwtPairCentroidsNormObserved)
    ...
    if not min_group_size:
        ...
        keep_iter = np.where(group_Dwt_s < min_group_size)[0].any()
        ...
        # log kma iteration
        print('KMA iteration info:')
        for rr in range(len(group_Dwt_s)):
            print('  cluster: {0}, size: {1}'.format(rr, group_Dwt_s[rr]))
        print('Try again: ', keep_iter)
        print('Total attemps: ', count_iter)
        print()
...
print('Total clustered days: {}'.format(np.sum(group_Dwt_s)))
