...
def return_value(sample_real, threshold, alpha, block_size, return_period,
   ...
   ny = block_size  # defining how many observations will be a block (usually anual)
   ...
   # p = np.arange(i_initial, len(np.asarray(sample_over_thresh))) / (len(np.asarray(sample_over_thresh)))  # Getting Plotting Position points
   N = 1 / (ny * (1 - p))  # transforming plotting position points to years
   # N = 1 / (len(sample) * (1 - p))  # transforming plotting position points to years
   # N = 1 / ((1 - p))  # transforming plotting position points to years
...
with open(r"realWavesTuk.pickle", "rb") as input_file:
...
for yy in range(50):
   ...
   simHsClimatologyMaxTemp = np.nan * np.ones((len(years),int(366)))
   ...
   for hh in years:
      ...
      numDays = 1
      c = 0
      for qq in range(int(365/numDays)):
         # tempSim = np.where((simYearlyDf[c:(c+numDays*24)]>0))
         # tempOG = np.where((ogYearlyDf[c:(c+numDays*24)]>0))
         # if len(tempSim[0]) > (numDays*18):
         simHsClimatologyMaxTemp[yCounter,qq] = np.nanmax(simYearlyDf[c:(c+numDays*24)])
   ...
   futureHsClimatologyMaxTemp[:,:,yy] = simHsClimatologyMaxTemp
...
for hh in range(50):
   ...
   # avgMax[putInZeros] = np.zeros((len(putInZeros[0])))
   #
   # avgMax2=futureHsClimatologyMaxTemp[hh,275:306,:]
   # putInZeros2 = np.where(np.isnan(avgMax2))
   # avgMax2[putInZeros2] = np.zeros((len(putInZeros2[0])))
   #
   # avgMax3 = futureHsClimatologyMaxTemp[hh, 306:337, :]
   # putInZeros3 = np.where(np.isnan(avgMax3))
   # avgMax3[putInZeros3] = np.zeros((len(putInZeros3[0])))
   #
   # avgMax4 = futureHsClimatologyMaxTemp[hh, 337:368, :]
   # putInZeros4 = np.where(np.isnan(avgMax4))
   # avgMax4[putInZeros4] = np.zeros((len(putInZeros4[0])))
   #
   # tempAvgMax.append(np.nanmean(np.nanmean(avgMax,axis=0)))
   # tempAvgMax2.append(np.nanmean(np.nanmean(avgMax2,axis=0)))
   # tempAvgMax3.append(np.nanmean(np.nanmean(avgMax3,axis=0)))
   # tempAvgMax4.append(np.nanmean(np.nanmean(avgMax4,axis=0)))
   ...
   ciArrayFutureSept.append(sim['CI'])
...
ax1.text(-0.05, 1.03, 'a.', transform=ax1.transAxes, size=12, weight='bold')
ax2.text(-0.05, 1.03, 'b.', transform=ax2.transAxes, size=12, weight='bold')


ax11 = plt.subplot2grid((3,2),(2,0))
...
for i in range(51):
    ...
    whatsNanPerDay = 1-np.sum(whatsNan, axis=1)/250
    whatsNanPerYear = (366-np.sum(whatsNan, axis=0))/366
    whatsNanTotaled[i] = np.mean((366-np.sum(whatsNan, axis=0))/366)

    # whatsNanTotaled[i] = np.mean(1-np.sum(whatsNan, axis=1)/250)
    # whatsNanUpper[i] = np.percentile(1-np.sum(whatsNan, axis=1)/250,90)
    # whatsNanLower[i] = np.percentile(1-np.sum(whatsNan, axis=1)/250,10)
    whatsNanUpper[i] = np.percentile((366-np.sum(whatsNan, axis=0))/366,90)
    whatsNanLower[i] = np.percentile((366-np.sum(whatsNan, axis=0))/366,10)
...
for i in range(51):
    ...
    # whatsNanTotaled[i] = np.mean(np.sum(whatsNan, axis=0)/250)
    # whatsNanUpper[i] = np.percentile(np.sum(whatsNan, axis=0)/250,90)
    # whatsNanLower[i] = np.percentile(np.sum(whatsNan, axis=0)/250,10)
    ...
    for qq in range(int(366 / (numDays/4))):
       ...
       c = int(c+(numDays/4))
    if i < 10:
       ax11.plot(np.arange(12, len(tempData)) * (numDays / 4), tempData[12:], label=i)
    else:
...
ax11.text(-0.05, 1.03, 'e.', transform=ax11.transAxes, size=12, weight='bold')
ax10.text(-0.05, 1.03, 'd.', transform=ax10.transAxes, size=12, weight='bold')
...
ax111 = plt.subplot2grid((3,2),(1,0))
...
for qq in range(len(zNArray)):
   ...
   ax111.plot(yearArray[qq],zNArray[qq],color=color,alpha=0.25)#color=[0.5,0.5,0.5],alpha=0.5)
...
historicalPlotTime = 1/np.flip(np.arange(1,1+len(historical['sample_over_thresh']))/45)

ax111.plot(historical['year_array'], historical['CI_z_N_high_year'], linestyle='--', color='red', alpha=0.8, lw=0.9, label='Confidence Bands')
ax111.plot(historical['year_array'], historical['CI_z_N_low_year'], linestyle='--', color='red', alpha=0.8, lw=0.9)
ax111.plot(historical['year_array'], historical['z_N'], color='red', label='Theoretical Return Level')
ax111.scatter(historicalPlotTime, historical['sample_over_thresh'], color='red',label='Empirical Return Level',zorder=10,alpha=0.5)
ax111.set_xscale('log')
ax111.set_xlabel('Return Period (years)')
ax111.set_ylabel('Max Storm Hs (m)')
# plt.title('Return Level Plot')
ax111.set_xlim([0.25,46])
ax111.set_ylim([3.5,7.5])
plt.legend()

ax111.text(-0.05, 1.03, 'c.', transform=ax111.transAxes, size=12, weight='bold')
ax111.set_title('TESLA Historical Simulations',weight='bold')
...
ax1111 = plt.subplot2grid((3,2),(2,1))
ax1111.set_prop_cycle(plt.cycler('color', plt.cm.jet(np.linspace(0, 1, 51))))
labels = []
whatsNanTotaled = np.nan*np.ones((51,))
whatsNanUpper = np.nan*np.ones((51,))
whatsNanLower = np.nan*np.ones((51,))
...
for i in range(51):
    ...
    for qq in range(int(366 / (numDays/4))):
       ...
       if len(np.where(np.isnan(subsetOfData))[0]) > 4:
         ...
         tempData[qq] = np.nanmean(subsetOfData)
       c = int(c+(numDays/4))
    # ax1111.plot(np.arange(0,len(tempData))*(numDays/4), tempData, label=i)
    if i < 10:
       ax1111.plot(np.arange(12, len(tempData)) * (numDays / 4), tempData[12:], label=i)
    else:
       ax1111.plot(np.arange(0,len(tempData))*(numDays/4), tempData, label=i)
    # c = c + 10
...
#                     183,214,244,275,305,336])
# ax11.xaxis.set_ticklabels(['Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec','Jan', 'Feb'])
ax1111.xaxis.set_ticks([1,61,122,
                     183,244,305])
ax1111.xaxis.set_ticklabels(['Mar','May','Jul','Sep','Nov','Jan'])

# cax = ax1111.inset_axes([20, 15, 110, 3], transform=ax1111.transData)
#
...
ax1111.set_ylabel('Mean Dm (deg)',weight='bold')
ax1111.set_xlabel('Month',weight='bold')

ax1111.text(-0.05, 1.03, 'f.', transform=ax1111.transAxes, size=12, weight='bold')
ax1111.set_title('TESLA Future Simulations',weight='bold')
...
for i in range(51):
    ...
    for qq in range(int(366 / (numDays/4))):
       ...
       if len(np.where(np.isnan(subsetOfData))[0]) > 4:
         ...
         tempData[qq] = np.nanmean(subsetOfData)
       c = int(c+(numDays/4))
    cumWP.append(np.nansum(tempData))
...
for qq in range(len(zNArray)):
   ...
   color = colormap(normalize(colorparam[qq]))
   plt.plot(yearArray[qq],zNArray[qq],color=color,alpha=0.25)#color=[0.5,0.5,0.5],alpha=0.5)
...
plt.plot(historical['year_array'], historical['z_N'], color='red', label='Theoretical Return Level')
plt.scatter(historicalPlotTime, historical['sample_over_thresh'], color='red',label='Empirical Return Level',zorder=10,alpha=0.25)
plt.xscale('log')
plt.xlabel('Return Period (years)')
plt.ylabel('Max Storm Hs (m)')
# plt.title('Return Level Plot')
plt.xlim([0.25,46])
plt.ylim([3.5,7.5])
plt.legend()
