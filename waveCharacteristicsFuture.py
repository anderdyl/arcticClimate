...
where360 = np.where((dmCombined > 360))
dmCombined[where360] = dmCombined[where360] - 360
whereNeg = np.where((dmCombined < 0))
dmCombined[whereNeg] = dmCombined[whereNeg] + 360
...
simYearlyMax = np.nan * np.ones((100,29))
yearArray = []
zNArray = []
ciArray = []
for hh in range(100):
   # file = r"/home/dylananderson/projects/atlanticClimate/Sims/simulation{}.pickle".format(hh)
   file = r"/media/dylananderson/Elements/SimsArctic/simulation{}.pickle".format(hh)
...
simMaxHs = np.nan*np.ones((100,29))
# simMaxHsNotInterped = np.nan*np.ones((50,100))
for hh in range(100):
   simMaxHs[hh,:] = np.sort(simYearlyMax[hh,0:29])
...
simReturnPeriod = np.flipud(29/np.arange(1,30))
