...
with open(r"futureSimulations500Chopped49.pickle", "rb") as input_file:
...
for simNum in range(500):
    ...
    simsPickle = ('/media/dylananderson/Elements/SimsArctic/simulation{}.pickle'.format(simNum))

    outputSims= {}
    outputSims['futureSimulationData'] = simDataInterp.T
    outputSims['df'] = df
...
hs[where0] = np.nan
ax1.plot(hourlyTime,hs,color=dwtcolors[27])
ax1.set_ylabel('Hs (m)')
ax2 = plt.subplot2grid((4,1),(1,0),rowspan=1,colspan=1)
tp = simDataInterp[1,:]
where0 = np.where((tp < 0.5))
tp[where0] = np.nan
ax2.plot(hourlyTime,tp,color=dwtcolors[27])
ax2.set_ylabel('Tp (s)')
...
ax3.plot(hourlyTime,dm,color=dwtcolors[27])
ax3.set_ylabel('Dm (deg)')
ax4 = plt.subplot2grid((4,1),(3,0),rowspan=1,colspan=1)
ss = simDataInterp[3,:]
where0 = np.where((ss == 0))
ss[where0] = np.nan
ax4.plot(hourlyTime,ss,color=dwtcolors[27])
ax4.set_ylabel('SWL (m)')
ax4.set_xlabel('')
