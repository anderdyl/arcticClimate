...
with open(r"ice18FutureSimulations1000Tuk.pickle", "rb") as input_file:
...
with open(r"historicalNTRDataTuk.pickle", "rb") as input_file:
...
wavesDir = '/Volumes/macDrive/arcticSites/Tuk'

wavesFiles = sorted_alphanumeric([filename for filename in os.listdir(wavesDir) if filename.startswith("waves")])
import xarray as xr
counter = 0
for fff in wavesFiles:
    print('loading {}'.format(fff))
    ds = xr.open_dataset(os.path.join(wavesDir, fff))  # ,engine='netcdf4')
...
windsDir = '/Volumes/macDrive/arcticSites/Tuk'

windsFiles = sorted_alphanumeric([filename for filename in os.listdir(windsDir) if filename.startswith("winds")])
import xarray as xr
counter = 0
for fff in windsFiles:
    print('loading {}'.format(fff))
    ds = xr.open_dataset(os.path.join(windsDir, fff))  # ,engine='netcdf4')
...
Tp = Tp.flatten()
Dm = Dm.flatten()
hourTime = np.asarray([datetime.utcfromtimestamp((qq - np.datetime64(0, 's')) / np.timedelta64(1, 's'))for qq in tC])
endTime = hourTime[0]
startTime = hourTime[-1]
#
# import sys
...
wh = Hs[beginTime[0][0]:endingTime[0][0]+24]
tp = Tp[beginTime[0][0]:endingTime[0][0]+24]
dm = Dm[beginTime[0][0]:endingTime[0][0]+24]
u10 = u10[beginTime[0][0]:endingTime[0][0]+24]
v10 = v10[beginTime[0][0]:endingTime[0][0]+24]
t2m = t2m[beginTime[0][0]:endingTime[0][0]+24]
...
time_all = np.asarray([datetime.utcfromtimestamp((qq - np.datetime64(0, 's')) / np.timedelta64(1, 's'))for qq in tC])[beginTime[0][0]:endingTime[0][0]+24]
# time_all = tC
...
realWavesPickle = 'realWavesTuk.pickle'
        ...
        if len(waveInd[0]) > 0:
            ...
            while tempHydroLength > 1:
                ...
                tempDict['cop'] = np.asarray([np.nanmin(wh[waveInd[0]]), np.nanmax(wh[waveInd[0]]),
                                              ...
                                              #np.nanmean(ssr[waveInd[0]]),
                                              np.nanmean(t2m[waveInd[0]]),
...
for i in range(len(np.unique(bmuGroup))):
    ...
    for kk in range(len(tempHydros)):
        dataCop.append(list([tempHydros[kk]['hsMax'], tempHydros[kk]['hsMin'], tempHydros[kk]['tpMax'],
                             ...
                             #tempHydros[kk]['ssrMean'],
                             tempHydros[kk]['t2mMean'],
                             tempHydros[kk]['fetch'], tempHydros[kk]['ntrMean'],# tempHydros[kk]['sstMean'],
                             tempHydros[kk]['t2mMax'],tempHydros[kk]['t2mMin'],len(tempHydros[kk]['time']), kk]))
...
normHydrosPickle = 'normalizedWaveHydrographsTuk.pickle'
...
hydrosPickle = 'waveHydrographsTuk.pickle'
...
copPickle = 'hydrographCopulaDataTuk.pickle'
...
historicalPickle = 'historicalDataTuk.pickle'
