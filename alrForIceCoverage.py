...
bmus = bmus[151:-214]+1
bmus_dates = iceDateTimes[151:-214]
...
cov_3 = cov_Temp.temp.values.reshape(-1,1)
...
p_mat_output = ('/media/dylananderson/Elements/NC_climate/testALR/testOfIceW36but2PCsNAO_y{0}s{1}.h5'.format(
        sim_years, sim_num))
import h5py
...
samplesPickle = 'iceTempWith2PCsNAO36DWTsSimulations100.pickle'
...
bmus_dates_months = np.array([d.month for d in dates_sim[0:730*2]])
bmus_dates_days = np.array([d.day for d in dates_sim[0:730*2]])
...
timeAsArray = np.array(slpDatetimes)
plt.figure()
...
for qq in range(len(np.unique(bmus))):
    ...
    ax1.plot(np.array(slpDatetimes)[getBMUS[0]],qq*np.ones((len(temp),)),'.',color=dwtcolors[iceDWTs['kma_order'][qq]])
...
for qq in range(len(np.unique(bmus))):
    ...
    tempTime = timeAsArray[getBMUS]
    ax2.plot(np.array(slpDatetimes)[getBMUS[0]],qq*np.ones((len(temp[0]),)),'.',color=dwtcolors[iceDWTs['kma_order'][qq]])
